...
xrdp_painter_send_dirty(struct xrdp_painter *self)
{
    ...
    if (self->wm->painter_codec_handle)
    {
        ...
        int tiles_written = rfxcodec_encode(self->wm->painter_codec_handle,
        ...
        LOG_DEVEL(LOG_LEVEL_DEBUG, "xrdp_painter_send_dirty: sending RFX encoding of %d tiles\n", tiles_written);
        ...
        libxrdp_fastpath_send_surface(self->session,
                                      ...
                                      encoding_bytes, 0, 0,
                                      width, height, 32,
                                      self->session->client_info->rfx_codec_id,
                                      width, height);
